...
- **Chart display options** — open **Options** (gear icon in the toolbar) to choose **Zoom Step** (**Fine (1%)** default, **Small (~10%)**, **Medium (~20%)**, or **Large (~25%)**) for zoom buttons, keyboard **+**/**−**, and scroll wheel; hide chart indicators per layout (North: bhava numbers in black corner boxes; South: lagna line, yellow bhava numbers, black rashi numbers), or enable **Save Chart Only** so the same Save button exports only the chart area — fits the chart, ignores zoom/pan, includes Grahas and on-chart Annotations, leaves out anything outside the chart boundary, uses a transparent background, and omits the watermark. When **Save Chart Only** is on, the transparency toolbar toggle turns on and stays locked; turning the option off restores a white background export. Preferences are saved in this browser, included when you **Save Session**, and are not tracked by undo/redo.
...
- **Zoom step options** — **Options** → **Zoom Step**: **Fine (1%)** default (one percent per step), **Small (~10%)**, **Medium (~20%)**, and **Large (~25%)** for zoom buttons, keyboard **+**/**−**, and scroll wheel; saved in this browser and included in session files.
