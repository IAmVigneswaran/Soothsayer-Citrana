...
3. [CSS and Layout](#css-and-layout-stylescss---3220-lines)
...
6. [Core Components Architecture](#core-components-architecture)
   - [Main Application (citrana-app.js)](#main-application-citrana-appjs---2248-lines)
   - [Canvas Hints (citrana-canvas-hints.js)](#canvas-hints-citrana-canvas-hintsjs---397-lines)
   - [History Engine (citrana-history.js)](#history-engine-citrana-historyjs---94-lines)
   - [Chart Coordinator](#chart-coordinator-citrana-chart-coordinatorjs---322-lines)
   - [Citrana Zoom](#citrana-zoom-citrana-zoomjs---58-lines)
   - [South Indian Chart Template](#south-indian-chart-template-citrana-chart-templates-southjs---989-lines)
   - [North Indian Chart Template](#north-indian-chart-template-citrana-chart-templates-northjs---1015-lines)
   - [Graha System](#graha-system-citrana-planet-systemjs---1000-lines)
   - [Citrana Arrow](#citrana-arrow-citrana-arrowjs---185-lines)
   - [Citrana Color Picker](#citrana-color-picker-citrana-colorpickerjs---388-lines)
   - [Citrana Device](#citrana-device-citrana-devicejs---39-lines)
   - [Citrana Rashis](#citrana-rashis-citrana-rashisjs---49-lines)
   - [Citrana Selection](#citrana-selection-citrana-selectionjs---99-lines)
   ...
   - [Drawing Tools](#drawing-tools-citrana-drawing-toolsjs---3270-lines)
   - [Context Menu](#context-menu-citrana-context-menujs---742-lines)
   - [Citrana Items Menu](#citrana-items-menu-citrana-items-menujs---861-lines)
   - [Citrana Session](#citrana-session-citrana-sessionjs---225-lines)
   - [Edit UI](#edit-ui-citrana-edit-uijs---1030-lines)
...
11. [Customisation Guidelines](#customisation-guidelines)
...
## CSS and Layout (styles.css - ~3270 lines)
...
- Context Menu row: `.items-row-context-menu-on` (green tint) / `.items-row-context-menu-off` (red tint); **Graha Library** row reuses same On/Off row tint (`orbit` icon; independent of Presentation View); **Reset Graha Library Position** uses neutral row styling (`orbit` row icon; `move` action button; meta **Default Layout**)

**Confirmation modal (`#confirmation-modal`):** Modes via `confirmation-modal--confirm|alert|open|save-as` on the same shell — destructive confirm (Yes/No + warning), single-action notice (OK), **Open Session** intro (**Choose file** / Cancel), **Save As** (`#confirmation-filename-input`, `#confirmation-filename-error`; base name pre-selected, full extension preserved). Styles: `.confirmation-filename-field`, `.confirmation-filename-input`, `.confirmation-filename-error`.
...
| `assets/js/citrana-drawing-tools.js` | ~3270 | Drawing tools implementation |
| `assets/js/citrana-edit-ui.js` | ~1030 | Edit interface controls |
| `assets/js/citrana-history.js` | ~94 | Unified undo/redo timeline |
| `assets/js/citrana-items-menu.js` | ~861 | Canvas Items panel — chart/Bhava/Graha/Annotation actions; Section Anchors; Context Menu and Graha Library toggles; **Reset Graha Library Position**; list-only scroll; mobile anchor fades |
| `assets/js/citrana-laser.js` | ~248 | Ephemeral laser pointer Canvas overlay |
| `assets/js/citrana-planet-system.js` | ~1000 | Graha library and drag-drop; panel reposition and reset; `.graha-library-dragging`; visibility toggle; dots-bar swipe; chevron hints |
| `assets/js/citrana-rashis.js` | ~49 | Rashi names, Lucide zodiac icons, grid numbers |
| `assets/js/citrana-selection.js` | ~99 | Selection Pill |
| `assets/js/citrana-session.js` | ~270 | Save/open `.citrana.json` session files; `normalizeDownloadFileName()`; optional `download(session, fileName)`; includes `zoomStep` in options |
| `assets/js/citrana-zoom.js` | ~58 | Zoom step presets (`CitranaZoom`) for buttons, keyboard, and scroll wheel |
...
| `assets/images/` | 19 files | Logos, demo GIFs, browser screenshot, canvas hint PNGs (`hint-*.png`) |
...
| `AGENT.md` | ~1355 | Comprehensive documentation |
| `ARCHITECTURE.md` | ~600 | System architecture and data flows |
| `.cursorrules` | ~1340 | Cursor IDE configuration |
| `CHANGELOG.md` | ~109 | Version history |
| `README.md` | ~249 | Project readme |
| `LICENSE` | — | MIT License |
...
3. `citrana-canvas-hints.js` — `CitranaCanvasHints` (before app init; uses `app` reference after `init()`)
4. `citrana-rashis.js` — `CitranaRashis`
5. `citrana-selection.js` — `CitranaSelection` (uses `CitranaDevice`)
6. `citrana-annotation-fonts.js`
7. `citrana-chart-templates-south.js` — uses `CitranaRashis`, `CitranaDevice`
8. `citrana-chart-templates-north.js` — same
9. `citrana-zoom.js` — `CitranaZoom` (before coordinator and session)
10. `citrana-chart-coordinator.js` — templates + `CitranaZoom`
11. `citrana-planet-system.js` — coordinator
12. `citrana-arrow.js` — before color picker
13. `citrana-colorpicker.js`
14. `citrana-laser.js` — before drawing tools
15. `citrana-drawing-tools.js`
16. `citrana-edit-ui.js`
17. `citrana-context-menu.js` — before Canvas Items menu
18. `citrana-items-menu.js`
19. `citrana-history.js`
20. `citrana-session.js` — uses `CitranaZoom`
21. `citrana-app.js` — **always last**; creates `window.app`
...
- Initialises **Canvas onboarding hints** (`CitranaCanvasHints`) and refreshes on resize, Presentation View, and Graha library layout changes
...
- `showConfirmationDialog()` / `showAlertDialog()` / `showOpenSessionDialog()` / `showSaveAsDialog()` / `_setConfirmationModalLayout()`: Reuse `#confirmation-modal` for confirm, alert, open, and save-as modes; `#confirmation-filename-input` for Save As; `getModalInitialFocusElement()` focuses filename field in save-as mode
...
### Canvas Hints (citrana-canvas-hints.js - ~397 lines)
Blank-canvas onboarding overlay until the user creates content.

Key Responsibilities:
- Renders fixed DOM overlay (`.citrana-canvas-hints`) with PNG hint images (shared 2084×501 artboard)
...
Key Methods:
- `init(app)`: Create overlay DOM, bind resize/`visualViewport` listeners
- `update()` / `scheduleUpdate()`: Show/hide root, position hints, toggle welcome z-index class
- `dismiss()`: Hide overlay until page reload
- `shouldShow()` / `isCanvasEmpty()`: Gated on `app.hasSessionContent()`, Presentation View, and dismiss flag

Wired from `app.setupComponents()`, `notifyCanvasContentCreated()`, `refreshCanvasHints()`, `togglePresentationView()`, `handleResize()`, chart coordinator create, Graha drop, and drawing tools (non-laser).
...
### Chart Coordinator (citrana-chart-coordinator.js - ~322 lines)
...
### Citrana Zoom (citrana-zoom.js - ~58 lines)
Shared zoom step math for toolbar buttons, keyboard **+**/**−**, scroll wheel, and session files.

Key Responsibilities:
- Resolve and validate zoom step (`fine`, `small`, `medium`, `large`)
- Compute next stage scale with clamping (`MIN_SCALE` 0.1, `MAX_SCALE` 5)

Key Methods:
- `resolveZoomStep(value)` — normalise step string (default `fine`)
- `computeNextScale(oldScale, direction, zoomStep)` — `fine` = ±1%; other steps use multipliers (~10%, ~20%, ~25%)

Wired from `citrana-app.js` (`handleWheel`, `setZoomStep`), `citrana-chart-coordinator.js` (`zoomIn`/`zoomOut`), and `citrana-session.js` (`options.zoomStep` on save/open).
...
### North Indian Chart Template (citrana-chart-templates-north.js - ~1015 lines)
...
### Graha System (citrana-planet-system.js - ~1000 lines)
...
- **Panel drag:** drag `.planet-library-header` to reposition; `setGrahaLibraryDragging()` applies `.graha-library-dragging` (z-index **10001** above toolbar/zoom/Help/About while dragging; below modals)
- **Reset position:** `resetGrahaLibraryPosition()` via Canvas Items → **Reset Graha Library Position**; clears inline `left`/`top`/`transform`/height locks so CSS defaults apply (top-left desktop, bottom-centre mobile); no confirmation; not undoable; works when library is hidden
...
### Drawing Tools (citrana-drawing-tools.js - ~3270 lines)
...
### Citrana Items Menu (citrana-items-menu.js - ~861 lines)
...
- **Canvas**: **Clear Selection** (`app.clearCanvasSelection()`), **Context Menu** toggle (On/Off meta; green `.items-row-context-menu-on` / red `.items-row-context-menu-off` row tint; `square-menu` / `power` / `power-off` icons), **Graha Library** toggle (same On/Off row tint; `orbit` row icon; `toggle-graha-library` → `PlanetSystem.toggleGrahaLibrary()`), **Reset Graha Library Position** (`orbit` row icon; `move` action button; meta **Default Layout**; `reset-graha-library-position` → `PlanetSystem.resetGrahaLibraryPosition()`; closes panel; no confirmation)
...
### Citrana Session (citrana-session.js - ~270 lines)
...
### Edit UI (citrana-edit-ui.js - ~1030 lines)
...
**Not tracked:** active tool, Bhava highlight, Graha library page, modals, chart indicator visibility preferences, Save Chart Only export preference, **Zoom Step** preference, **laser pointer strokes**, **Presentation View**.
...
- **Zoom Step**: **Fine (1%)** default; **Small (~10%)**, **Medium (~20%)**, and **Large (~25%)** for zoom buttons, keyboard **+**/**−**, and scroll wheel when zoom is unlocked (`CitranaZoom.computeNextScale()`)
...
- **Persistence**: `localStorage.citrana_north_hide_indicators`, `citrana_south_hide_indicators`, `citrana_save_chart_only`, and `citrana_zoom_step` (`'1'` when indicators hidden or Save Chart Only on; zoom step key removed when `fine`); defaults are indicators visible, full viewport export, and **Fine** zoom step
- **Application**: `app.setNorthHideIndicators()` / `setSouthHideIndicators()` / `setSaveChartOnly()` / `setZoomStep()`; indicator prefs reapply on chart create and Lagna changes via `apply*IndicatorsPreference()`
...
- **Canvas hints:** On a blank canvas, illustrated tips highlight the Graha Library, toolbar, zoom bar, and Help (staggered fade-in); centred logo + start message; hidden after first chart/Graha/annotation or in Presentation View
- **Canvas Items**: `#items-menu-btn` or **I** — chart/Bhava/Graha/Annotation actions; pinned Section Anchors; **Clear Selection**, **Context Menu**, **Graha Library** (On/Off with green/red row tint), and **Reset Graha Library Position**; row highlight syncs with canvas selection; desktop and touch workflow
...
### Export & Sharing
- **Full viewport export** (default): `stage.toDataURL({ pixelRatio: 2 })` on entire stage; **Save As** dialog first; 100px padding and watermark via `finalizeExportImage()`; respects current zoom/pan and `#toggle-transparency-btn`; progress dialog **Exporting Chart**
...
- **Save Session**: **Save As** dialog (default `citrana-session-{timestamp}.citrana.json`), then `.citrana.json` download with chart, Grahas, Annotations, and Options (including Zoom Step); progress dialog during save; save to any cloud storage to resume on another device
...
- `localStorage` is used for preferences: welcome modal seen (`citrana_welcome_seen`), chart indicator toggles (`citrana_north_hide_indicators`, `citrana_south_hide_indicators`), Save Chart Only export (`citrana_save_chart_only`), Zoom Step (`citrana_zoom_step`), context menu enable (`citrana_context_menu_enabled` — touch-primary default off until set), Graha Library visibility (`citrana_graha_library_enabled` — default on), optional `citrana_debug` opt-out
